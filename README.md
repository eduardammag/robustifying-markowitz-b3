...
O projeto separa o pipeline em sete módulos principais:

- `src/data.py`: download, limpeza, retornos e janelas moveis.
- `src/estimators.py`: estimadores de covariancia classicos, shrinkage e robustos.
- `src/optimization.py`: otimizacao, PGD e projecoes de pesos.
- `src/portfolios.py`: construcao das carteiras.
- `src/backtest.py`: rolling window, rebalanceamento e custos.
- `src/metrics.py`: metricas de performance, risco e pesos.
- `src/plots.py`: figuras e tabelas finais.
...
Execute o pipeline com o universo Ibovespa:
...
com custo proporcional de transacao de 50 bps (`0.005`). Os resultados ficam em:

- `data/results/window_252/`
...
Para usar outra lista congelada, rode:

```bash
python run.py --tickers-file data/raw/tickers_ibovespa.txt
```

Para testar o pipeline sem internet:
...
A carteira `GMV Robust` segue a logica central do artigo: em vez de estimar e inverter uma
...
procedimento spectral-center/HLZ: a rotina alterna entre a direcao de maior dispersao e
...
O turnover reportado em `outputs/tables/metrics_*.csv` separa:

- `turnover`: soma absoluta dos trades para rebalancear a carteira apos o drift de precos.
- `target_turnover`: mudanca entre pesos-alvo, limpando o efeito dos precos.

`GMV Nonlinear Shrinkage` usa uma implementacao espectral interna inspirada em
Ledoit-Wolf: os autovetores da covariancia amostral sao preservados e os autovalores
...
- `turnover_pvalue_vs_robust`: t-test pareado para turnover.
- `target_turnover_pvalue_vs_robust`: t-test pareado para target turnover.
- `sharpe_pvalue_vs_robust`: teste HAC/Newey-West por delta method para Sharpe.
- `variance_pvalue_vs_robust`: teste HAC/Newey-West por delta method para variancia.

## Validacao

O reposititorio inclui scripts de auditoria:
...
Ledoit-Wolf, exporte a matriz de referencia oficial para CSV e rode:
